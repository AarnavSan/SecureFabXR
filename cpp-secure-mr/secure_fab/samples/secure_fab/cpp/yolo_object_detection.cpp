...
static bool LoadModelData(const std::string& filePath, std::vector<char>& modelData) {
  ...
  return true;
    ...
    YoloDetector::YoloDetector(const XrInstance& instance, const XrSession& session)
        ...
        Log::Write(Log::Level::Error, "=== CONSTRUCTOR START ===");

        if (g_assetManager == nullptr) {
            Log::Write(Log::Level::Error, "g_assetManager is NULL!");
            Log::Write(Log::Level::Error, "Add: extern AAssetManager* g_assetManager;");
            return;
        }

        Log::Write(Log::Level::Info, "g_assetManager OK - continuing...");
...
void YoloDetector::RenderText(const std::shared_ptr<Pipeline>& pipeline,
                                  const std::shared_ptr<PipelineTensor>& textArrayTensor,
                                  const std::shared_ptr<PipelineTensor>& pointXYZ,
                                  const std::shared_ptr<PipelineTensor>& gltfPlaceholder,
                                  const std::shared_ptr<PipelineTensor>& scale,
                                  const std::shared_ptr<PipelineTensor>& score,
                                  const std::shared_ptr<PipelineTensor>& detectedClass) {  // ADD THIS PARAMETER
        ...
        // === VISIBILITY LOGIC: Score threshold AND Class match ===

        // Check 1: Score above threshold
        ...
        auto threshold = std::make_shared<PipelineTensor>(pipeline, TensorAttribute{.dimensions = {1},
                ...
                .usage = XR_SECURE_MR_TENSOR_TYPE_SCALAR_PICO,
                .dataType = XR_SECURE_MR_TENSOR_DATA_TYPE_FLOAT32_PICO});
        threshold->setData(reinterpret_cast<int8_t*>(new float[1]{0.15}), 1 * sizeof(float));
        (*pipeline).compareTo(*score > threshold, isDetected);

        // === ADD THIS: Check 2: Class matches target ===
        auto isCorrectClass = std::make_shared<PipelineTensor>(pipeline, TensorAttribute{.dimensions = {1},
                .channels = 1,
                .usage = XR_SECURE_MR_TENSOR_TYPE_SCALAR_PICO,
                .dataType = XR_SECURE_MR_TENSOR_DATA_TYPE_INT32_PICO});
        auto targetClassTensor = std::make_shared<PipelineTensor>(pipeline, TensorAttribute{.dimensions = {1},
                .channels = 1,
                .usage = XR_SECURE_MR_TENSOR_TYPE_SCALAR_PICO,
                .dataType = XR_SECURE_MR_TENSOR_DATA_TYPE_FLOAT32_PICO});
        targetClassTensor->setData(reinterpret_cast<int8_t*>(new float[1]{static_cast<float>(TARGET_CLASS_ID)}), 1 * sizeof(float));

        // Compare detected class with target class
        (*pipeline).compareTo(*detectedClass == targetClassTensor, isCorrectClass);

        // Combine both conditions: visible only if score > threshold AND class matches
        auto isVisible = std::make_shared<PipelineTensor>(pipeline, TensorAttribute{.dimensions = {1},
                .channels = 1,
                .usage = XR_SECURE_MR_TENSOR_TYPE_SCALAR_PICO,
                .dataType = XR_SECURE_MR_TENSOR_DATA_TYPE_INT32_PICO});
        (*pipeline).elementwise(Pipeline::ElementwiseOp::MULTIPLY, {isDetected, isCorrectClass}, isVisible);
        // ==========================================
        ...
        // === USE isVisible instead of isDetected ===
        ...
        renderCommand_Render->visible = isVisible;  // CHANGED from isDetected to isVisible
...
void YoloDetector::CreateSecureMrModelInferencePipeline() {
  ...
  // Sort scores directly without filtering - detection will show all objects
...
void YoloDetector::CreateSecureMrRenderingPipeline() {
        ...
        // === ONLY CREATE TENSORS FOR NUMBER_OF_OBJECTS (which is 1) ===
        ...
        // Extract only the first (and only) detection
        ...
        // Extract detected class for filtering
        auto class0 = std::make_shared<PipelineTensor>(m_secureMrRenderingPipeline, TensorAttribute{.dimensions = {1},
                .channels = 1,
                .usage = XR_SECURE_MR_TENSOR_TYPE_SCALAR_PICO,
                .dataType = XR_SECURE_MR_TENSOR_DATA_TYPE_FLOAT32_PICO});
        (*m_secureMrRenderingPipeline).assignment((*classesSelectPlaceholder1)[{{0, 1},{0, 1}}], class0);

        // === RENDER ONLY THE SINGLE DETECTION ===
        RenderText(m_secureMrRenderingPipeline, textArray0, pointXYZ0, gltfPlaceholderTensor, scale0, score0, class0);
    }
...
void YoloDetector::RunSecureMrMap2dTo3dPipeline() {
  m_secureMrMap2dTo3dPipeline->submit({{nmsBoxesPlaceholder1, nmsBoxesGlobal},
    ...
    void YoloDetector::RunSecureMrRenderingPipeline() {
        m_secureMrRenderingPipeline->submit({{gltfPlaceholderTensor, gltfAsset},  // Only one GLTF now!
